...
def downgrade():
    ...
    try:
        # 删除知识中心相关表（按依赖顺序）
        cursor.execute("DROP TABLE IF EXISTS knowledge_graph_edges")
        cursor.execute("DROP TABLE IF EXISTS knowledge_graph_nodes")
        cursor.execute("DROP TABLE IF EXISTS knowledge_graphs")
        cursor.execute("DROP TABLE IF EXISTS decision_tree_nodes")
        cursor.execute("DROP TABLE IF EXISTS decision_trees")
        
        conn.commit()
        print("知识中心相关表删除成功")
    except sqlite3.Error as e:
        print(f"删除知识中心相关表时出错: {e}")
    finally:
        conn.close()
